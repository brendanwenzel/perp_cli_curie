...
pub async fn process(args: OpenCommand) -> Result<()> {
    ...
    if args.long == Some(false) && args.short == Some(false) {panic!("Please specify either --long or --short. Use --help to see more information.");}
    if args.long == Some(true) && args.short == Some(true) {panic!("Please specify only one: --long or --short. Use --help to see more information.");}

    if args.input == Some(false) && args.output == Some(false) {panic!("Please specify either --input or --output. Use --help to see more information.");}
    if args.input == Some(true) && args.output == Some(true) {panic!("Please specify only one: --input or --output. Use --help to see more information.");}
...
fn price_to_sqrt(price: f64) -> Result<U256> {
    let sqrt_x96 = price.sqrt();
    let q96 = ethers::utils::format_units(U256::from(2).pow(U256::from(96)), 18)?.parse::<f64>()?;
    let limit_sqrt = ethers::utils::parse_ether(sqrt_x96 * q96)?;
    Ok(limit_sqrt)
}
...
mod tests {
    ...
    fn test_price_to_sqrt() -> Result<()> {
        ...
        assert_eq!(limit_sqrt, U256::from(2895646998769791 as u128).saturating_mul(U256::from(100000000000000 as u128)));
        Ok(())
    }
